...
MYFILES=(movies language genres business actors actresses directors producers miscellaneous)

for i in "${MYFILES[@]}"
do
    curl -o "/data/development/movie_files/"${i}".list.gz" "ftp://ftp.fu-berlin.de/pub/misc/movies/database/"${i}".list.gz"
    gunzip "/data/development/movie_files/"${i}".list.gz"
done
